...
const gridOptions: VxeTableGridOptions<SystemPositionApi.PositionInfo> = {
  columns: [
    ...(useColumns() as any),
    {
      field: 'action',
...
function onEdit(row: SystemPositionApi.PositionInfo) {
...
async function onDelete(row: SystemPositionApi.PositionInfo) {
  if (!row.id) return;

  const hideLoading = message.loading({
    content: $t('ui.actionMessage.deleting', [row.positionName]),
    duration: 0,
  ...
  try {
    ...
    message.success({
      content: $t('ui.actionMessage.deleteSuccess', [row.positionName]),
      key: 'action_process_msg',
    });
    onRefresh();
  } catch (error) {
