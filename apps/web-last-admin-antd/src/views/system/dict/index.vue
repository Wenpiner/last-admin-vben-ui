...
const gridOptions: VxeTableGridOptions<SystemDictApi.DictInfo> = {
  columns: [
    ...(useColumns() as any),
    {
      field: 'action',
        ...
        default: ({ row }) => {
          return h(TableAction, {
            ...
            actions: [
              {
                type: 'link',
                label: $t('system.dict.item.manage'),
                onClick: () => {
                  onManageDictItems(row);
                },
              },
              {
                type: 'link',
...
function onEdit(row: SystemDictApi.DictInfo) {
...
function onManageDictItems(row: SystemDictApi.DictInfo) {
  dictItemModalApi.setData(row).open();
}

async function onDelete(row: SystemDictApi.DictInfo) {
  if (!row.id) return;
