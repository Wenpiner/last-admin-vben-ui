...
import { deleteOauth, getOauthList } from '#/api/system/oauth';
...
const gridOptions: VxeTableGridOptions<SystemOauthApi.OauthInfo> = {
  columns: [
    ...(useColumns() as any),
    {
      fixed: 'right',
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        return await getOauthList({
          page, // 使用大的 pageSize 加载所有数据
          ...formValues,
        });
      },
    },
...
function onEdit(row: SystemOauthApi.OauthInfo) {
...
async function onDelete(row: SystemOauthApi.OauthInfo) {
  if (!row.id) return;

  const hideLoading = message.loading({
    content: $t('ui.actionMessage.deleting', [row.providerName]),
    duration: 0,
  ...
  try {
    ...
    message.success({
      content: $t('ui.actionMessage.deleteSuccess', [row.providerName]),
      key: 'action_process_msg',
    });
    onRefresh();
  } catch (error) {
